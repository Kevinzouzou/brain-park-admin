...
export const userTarget=`/api/userTargetAssociation/userTargetAssociationList?targetId=`;  //用户目标关联（各种报名,申请信息）
...
export const subComs=`/api/subjectComment/subjectCommentListByParkId/`+localStorage.getItem("parkId");  //查看评论（包括评论对应的回复信息）
export const delSubCom=`/api/subjectComment/deleteSubjectComment/`;  //删除评论（同时删除回复）
...
export const inspectUrl=`/api/inspectionRecord/inspectionRecordList?parkId=`+localStorage.getItem("parkId");  //设备巡检列表
export const equipUrl=`/api/parkEquipment/equipmentList?parkId=`+localStorage.getItem("parkId");  //设备列表
export const delEquip=`/api/parkEquipment/deleteEquipment/`;  //删除设备
export const addEquip=`/api/parkEquipment/addEquipment`;  //添加设备

export const replyUrl=`/api/propertyApplication/suggestionReply?parkId=`+localStorage.getItem("parkId");  //设备列表
